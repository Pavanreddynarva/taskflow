...
const statusLabel = (s) => ({ pending: 'Pending', 'in-progress': 'In Progress', completed: 'Completed' }[s] || s);
const priorityLabel = (p) => ({ low: 'Low', medium: 'Medium', high: 'High' }[p] || p);

const isOverdue = (task) =>
  task.status !== 'completed' && task.deadline && isPast(new Date(task.deadline));

const UserAvatar = ({ user }) => {
  if (!user) return <span style={{ color: 'var(--text-3)', fontSize: '0.8rem' }}>Unassigned</span>;
...
export default function TaskCard({ task, onStatusChange, onDelete, showProject = false }) {
      ...
      <div className="task-card-body">
        ...
        <div className="task-meta">
          {showProject && task.project && (
            <Link to={`/projects/${task.project._id}`} className="task-project-badge" style={{ '--project-color': task.project.color || 'var(--accent)' }}>
              ⬡ {task.project.name}
            </Link>
          )}
